...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        node_feature_list = torch.load('{}/{}{}'.format(path_file.node_data_path, case_name, self.walk_id))
        dense_raw_adj_list = torch.load('{}/{}'.format(path_file.adj_data_path, case_name))
        edge_label = torch.load('{}/{}'.format(path_file.label_data_path, case_name))
        return node_feature_list, dense_raw_adj_list, edge_label, case_name

def normalize(mx):
    """Row-normalize sparse matrix"""
    rowsum = mx.sum(dim=1).clone().detach()
...
def process_loaded_data(data, max_size, walk_id=None):
    ...
    N = node_feature_list.shape[0]
    if walk_id is None:
        walk_id = 0
        node_feature_list = torch.load('{}/{}_{}'.format(path_file.node_data_path, case_name[0], str(walk_id)))
